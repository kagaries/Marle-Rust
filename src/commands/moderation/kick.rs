...
#[poise::command(slash_command, guild_only, required_permissions = "KICK_MEMBERS", rename = "kick")]
pub async fn kick_command(
    ...
    if !guild.user_permissions_in(&ctx.guild_channel().await.unwrap(), &guild.member(ctx, ctx.framework().bot_id).await.unwrap()).kick_members() {
    ...
    guild.member(ctx, user.id).await?
        .kick_with_reason(ctx, &reason.unwrap_or("No reason provided.".to_string())).await?;
